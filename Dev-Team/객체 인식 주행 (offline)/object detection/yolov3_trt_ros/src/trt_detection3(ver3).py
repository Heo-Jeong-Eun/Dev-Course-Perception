...
TRT = '/home/nvidia/xycar_ws/src/lane_keeping_system/yolov3_trt_ros/src/model_epoch2000v3.trt'
NUM_CLASS = 8
INPUT_IMG = '/home/nvidia/xycar_ws/src/yolov3_trt_ros/src/video1_2.png'

xycar_image = np.uint8([0])
mapping_image = np.full((640, 640, 3), 255, dtype=np.uint8)
bridge = CvBridge()
...
class yolov3_trt(object):
    ...
    def detect(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.show_img:
                ...
                # draw_location(mapping_image, boxes, scores, classes, ALL_CATEGORIES)
    ...
    def get_location_and_distance(self, bbox):
        ...
        homography = np.load('/home/nvidia/xycar_ws/src/lane_keeping_system/yolov3_trt_ros/src/homography_xycar168_new.npy')
        ...
        # print("id: {} x: {} y: {} distance: {}".format(bbox.id, x, y, distance))
    ...
    def _write_message(self, detection_results, boxes, scores, classes):
        ...
        for box, score, category in zip(boxes, scores, classes):
            ...
            # draw_location(mapping_image, detection_msg.x, detection_msg.y,  detection_msg.id , ALL_CATEGORIES, bbox_color='red')
...
def draw_location(mapview, x, y, bbox_id, all_categories, bbox_color='red'):
    
    p = [200 * x, 200 * (y + 1.35)]
    cv2.circle(mapview, (int(p[1]), int(p[0])), 5, (0, 0, 255), 2)
    cv2.putText(mapview, all_categories[bbox_id], (int(p[1]), int(p[0])), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2, cv2.LINE_AA)
