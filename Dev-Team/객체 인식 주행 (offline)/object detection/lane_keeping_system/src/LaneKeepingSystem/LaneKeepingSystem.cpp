...
LaneKeepingSystem<PREC>::LaneKeepingSystem()
{
    ...
    mTrafficLightColorDetector = std::make_unique<GetTrafficLightColor<PREC>>(config);
...
void LaneKeepingSystem<PREC>::setParams(const YAML::Node& config)
{
    ...
    mStart = config["TRFFIC_LIGHT"]["IS_START"].as<bool>();
    ...
    mDynamicThresholdX = config["LIDAR"]["DYNAMIC_X_THRESHOLD"].as<PREC>();
    mDynamicThresholdY = config["LIDAR"]["DYNAMIC_Y_THRESHOLD"].as<PREC>();

    mSignSignalSecond = config["DETECTION"]["SIGN_LIFESECOND"].as<PREC>();
    mSignMiniSecond = config["DETECTION"]["MINI_LIFESECOND"].as<PREC>();
    ...
    mYoloResolution = config["IMAGE"]["YOLO_RESOLUTION"].as<PREC>();
    mTrafficSignColor = config["TRAFFIC_LIGHT"]["BLUE_COLOR"].as<int16_t>();
}
...
void LaneKeepingSystem<PREC>::run()
{
    ...
    std::string detectedTrafficSignLabel = "TRAFFIC_LIGHT";
    std::string prevDetectedTrafficSignLabel = "TRAFFIC_LIGHT";
    ...
    while (ros::ok())
    {
        ...
        if (!mDetectTrafficSigns.empty())
        {
            ...
            const int32_t minX = static_cast<int32_t>((std::get<1>(box) / mYoloResolution * static_cast<PREC>(mFrame.cols)) * 0.9);
            const int32_t minY = static_cast<int32_t>((std::get<2>(box) / mYoloResolution * static_cast<PREC>(mFrame.rows)) * 0.9);
            const int32_t maxX = static_cast<int32_t>((std::get<3>(box) / mYoloResolution * static_cast<PREC>(mFrame.cols)) * 1.1);
            const int32_t maxY = static_cast<int32_t>((std::get<4>(box) / mYoloResolution * static_cast<PREC>(mFrame.rows)) * 1.1);
        ...
        if (isStart && isStop) {
            detectedTrafficSignLabel = "TRAFFIC_LIGHT";
        }
        ...
        if (mLidarDetectBox.size() > 0)
        {
            ...
            for (auto bbox : mLidarDetectBox) {
                ...
                if (std::abs(std::get<0>(bbox)) <= mDynamicThresholdX && std::abs(y) <= mDynamicThresholdY) {
            ...
            PREC y = (std::get<3>(bbox) + std::get<1>(bbox)) / 2.f;
            PREC width = std::get<2>(bbox) - std::get<0>(bbox);
            PREC height = std::get<3>(bbox) - std::get<1>(bbox);
...
void LaneKeepingSystem<PREC>::trafficSignCallback(const yolov3_trt_ros::BoundingBoxes& message)
{
    ...
    for (auto boundingBox : message.bounding_boxes)
    {
        ...
        if (boundingBoxArea <= mMinBoundingboxArea || boxClass == 6) {
            continue;
        }

        if (boxClass != 4 && mMinProbability > boundingBox.probability)
        {
            continue;
        }
    ...
    else
    {
        while (!mDetectTrafficSigns.empty())
        {
            ...
            PREC signalLifeSecond = trafficSignLabel == 4 ? mSignMiniSecond : mSignSignalSecond;
            if (!isUpdate && timeDiff.toSec() <= signalLifeSecond)
            {
