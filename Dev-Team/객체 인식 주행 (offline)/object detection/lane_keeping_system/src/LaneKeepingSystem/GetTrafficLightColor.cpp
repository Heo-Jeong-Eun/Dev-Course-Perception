...
GetTrafficLightColor<PREC>::GetTrafficLightColor(const YAML::Node& config)
{
    setParams(config);
}
...
int16_t GetTrafficLightColor<PREC>::detectTrafficLightColor2(const cv::Mat& cropped_image)
{
    ...
    // TODO: MagicNumber redGreenRatio 녹색불 비율 수정
    if (redGreenRatio <= mRedGreenRatio) {
        result = mBlue;
    } else {
        result = mNonBlue;
    }
...
template <typename PREC>
void GetTrafficLightColor<PREC>::setParams(const YAML::Node& config)
{
    mRedGreenRatio = config["TRAFFIC_LIGHT"]["RED_GREEN_RATIO"].as<PREC>();
    mBlue = config["TRAFFIC_LIGHT"]["BLUE_COLOR"].as<int16_t>();
    mNonBlue = config["TRAFFIC_LIGHT"]["NON_BLUE_COLOR"].as<int16_t>();
}
