...
void HoughTransformLaneDetector<PREC>::setConfiguration(const YAML::Node& config)
{
    ...
    mLeftLineGap = config["HOUGH"]["LEFT_LINE_GAP"].as<int32_t>();
    mLeftLineForecast = config["HOUGH"]["LEFT_LINE_FORECAST"].as<int32_t>();
    mRightLineGap = config["HOUGH"]["RIGHT_LINE_GAP"].as<int32_t>();
    mRightLineForecast = config["HOUGH"]["RIGHT_LINE_FORECAST"].as<int32_t>();
...
std::pair<int32_t, int32_t> HoughTransformLaneDetector<PREC>::getLanePosition(const cv::Mat& image, bool runUpdate, std::string detection)
{
    ...
    // TODO: MagicNumber
    if (detection == "LEFT" && laneGap >= mLeftLineGap) {
        rightPosition =  leftPosition + mRightLineForecast;
        rightUpdated = true;
        std::cout << "LEFT DETECTED" << std::endl;
    }
    if (detection == "RIGHT" && laneGap >= mRightLineGap) {
        leftPosition =  rightPosition - mLeftLineForecast;
        leftUpdated = true;
