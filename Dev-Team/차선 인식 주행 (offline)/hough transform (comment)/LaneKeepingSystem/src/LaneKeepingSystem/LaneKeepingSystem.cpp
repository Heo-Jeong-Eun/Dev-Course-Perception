...
// LaneKeepingSystem가 아닌 경우 mPID, mMovingAverage, mHoughTransformLaneDetector -> delete
template <typename PREC>
LaneKeepingSystem<PREC>::~LaneKeepingSystem()
{
    delete mPID;
...
void LaneKeepingSystem<PREC>::run()
{
    ...
    { 
        ...
        // errorFromMid을 사용해 Angle 값을 출력, kXycarSteeringAangleLimit = 50
...
// subscribe로 받은 토픽을 callback에서 처리한다. 
template <typename PREC>
void LaneKeepingSystem<PREC>::imageCallback(const sensor_msgs::Image& message)
{
    // message == topic 
    // message.height * message.width == 640 * 480 / type / 초기값 / step
...
void LaneKeepingSystem<PREC>::speedControl(PREC steeringAngle)
{
    if (std::abs(steeringAngle) > mXycarSpeedControlThreshold)
    {
        ...
        mXycarSpeed -= (mDecelerationStep) * std::abs(steeringAngle);
...
void LaneKeepingSystem<PREC>::drive(PREC steeringAngle)
{
    ...
    std::cout << "mXycarSpeed: " << mXycarSpeed << std::endl;
